...
int CRX14_ADDR_var = 0xA8;

void rfidsetdev(int hey) {
    CRX14_ADDR_var = hey;
}
...
uchar init_rfid(void)
{
  ...
  dummy_tab[1]=0x10;
  if(!writecheck(CRX14_ADDR_var,dummy_tab,2)) return FALSE;
  //Read answer tab
  if(!readcheck(CRX14_ADDR_var,dummy_tab,1)) return FALSE;
...
uchar initiate_rfid(void)
{
  ...
  dummy_tab[2]=0x06;
  dummy_tab[3]=0x00;
  if(!writecheck(CRX14_ADDR_var,dummy_tab,4)) return FALSE;
  return TRUE;
}
...
uchar slot_marker_rfid(void)
{
  ...
  if(!writecheck(CRX14_ADDR_var,dummy_tab,1)) return FALSE;
  return TRUE;
}
...
uchar read_frame_rfid(uchar *data, uchar nb_bytes)
{
 ...
 //Select frame register
  ...
  if(!writecheck(CRX14_ADDR_var,dummy_tab,1)) return FALSE;
  //Read answer tab
  if(!readcheck(CRX14_ADDR_var,data,nb_bytes)) return FALSE;
  return TRUE;
}
...
uchar completion_rfid(void)
{
  ...
  dummy_tab[1]=0x01;
  dummy_tab[2]=0x0F;
  if(!writecheck(CRX14_ADDR_var,dummy_tab,3)) return FALSE;
  return TRUE;
}
...
void write_eeprom_rfid(uchar chip_id, uchar num_block, uchar *data, uchar num_bytes)
{
  ...
  //Send command
  while(write_i2c(CRX14_ADDR_var,dummy_tab,(uchar)(4+num_bytes))==FALSE)
    __no_operation();
}
...
void read_eeprom_rfid(uchar chip_id, uchar num_block, uchar *data, uchar num_bytes)
{
  ...
  dummy_tab[3]=num_block+7;
  //Send command
  while(write_i2c(CRX14_ADDR_var,dummy_tab,4)==FALSE)
    __no_operation();
