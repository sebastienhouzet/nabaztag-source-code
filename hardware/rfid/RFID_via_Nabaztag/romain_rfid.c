...
/************************************************************************************************
Le but est de retransformer le fichier RFID.c pour supporter plusieurs lecteurs RFID
L'adresse I²C du premier lecteur RFID est dans le fichier I2C.h (#define CRX14_ADDR		0xA0
L'adresse I²C du composant est formée de cette manière :
    b : 1010 aaax
	a > Les bits a sont définit par le schéma (pin du CRX14)
	x > Varie en fonction de la lecture ou de l'écriture
************************************************************************************************/

// On définit ici les adresses des lecteurs RFID (jusqu'à 8 lecteurs)
int CurrentRFID = 0;
uchar RFID[2] = { 0x00, 0x02 } ;

/************************************************************************************************
Les devices RFID possibles
*************************************************************************************************
RFID[] = 0x00; // b 0000 0000
RFID[] = 0x02; // b 0000 0010
RFID[] = 0x04; // b 0000 0100
RFID[] = 0x06; // b 0000 0110
RFID[] = 0x08; // b 0000 1000
RFID[] = 0x0A; // b 0000 1010
RFID[] = 0x0C; // b 0000 1100
RFID[] = 0x0E; // b 0000 1110
************************************************************************************************/
...
/************************************************************************************************
Functions for act between I²C and RFID (CRX14)
************************************************************************************************/
int readcheck(uchar addr_i2c, uchar *data, uchar nb_byte)
{
  int nmax=1000;
  while((nmax>0)&&(read_i2c(addr_i2c | RFID[CurrentRFID],data,nb_byte)==FALSE)){ nmax--;__no_operation();}
  if (!nmax) i2cerror=1;
  return nmax;
}
...
{
  int nmax=1000;
  while((nmax>0)&&(write_i2c(addr_i2c | RFID[CurrentRFID],data,nb_byte)==FALSE)){ nmax--;__no_operation();}
  if (!nmax) i2cerror=1;
  return nmax;
}
...
uchar init_rfid(void)
{
  ...
  dummy_tab[1]=0x10;
  if(!writecheck(CRX14_ADDR | RFID[CurrentRFID],dummy_tab,2)) return FALSE;
  
  //Read answer tab
  if(!readcheck(CRX14_ADDR | RFID[CurrentRFID],dummy_tab,1)) return FALSE;
...
void write_eeprom_rfid(uchar chip_id, uchar num_block, uchar *data, uchar num_bytes)
{
  ...
  //Send command
  while(write_i2c(CRX14_ADDR | RFID[CurrentRFID],dummy_tab,(uchar)(4+num_bytes))==FALSE)
    __no_operation();
}
...
void read_eeprom_rfid(uchar chip_id, uchar num_block, uchar *data, uchar num_bytes)
{
  ...
  dummy_tab[3]=num_block+7;
  //Send command
  while(write_i2c(CRX14_ADDR | RFID[CurrentRFID],dummy_tab,4)==FALSE)
    __no_operation();
